...
import (
	...
	"github.com/AayushVinayDev/broker/event"
	"github.com/AayushVinayDev/broker/logs"
...
func (app *Config) logItem(w http.ResponseWriter, entry LogPayload) {
	...
	if err != nil {
		log.Printf("Error making request to logger service: %v", err)
		app.errorJSON(w, err)
		return
	}
	...
	if response.StatusCode != http.StatusAccepted {
		log.Printf("Unexpected status code from logger service: %d", response.StatusCode)
		app.errorJSON(w, errors.New("unexpected status code from logger service"))
		return
	}
...
func (app *Config) sendMail(w http.ResponseWriter, msg MailPayload) {
	...
	response, err := client.Do(request)
	if err != nil {
		log.Printf("Error making request to mail service: %v", err)
		app.errorJSON(w, err)
		return
	}
	defer response.Body.Close()
	...
	payload.Error = false
	payload.Message = "Authenticated successfully"
	payload.Data = "Message sent to " + msg.To
...
func (app *Config) logItemViaRPC(w http.ResponseWriter, l LogPayload) {
	...
	rpcPayload := RPCPayload{Name: l.Name, Data: l.Data}
